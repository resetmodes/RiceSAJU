...
const TEN_GODS = {
  비견: ['오늘은 비견의 날. 다들 먹고 싶은 게 같아서 눈치싸움 남', '경쟁의 기운. 맛있는 반찬은 먼저 집어', '어깨 나란히 하는 날. 메뉴 통일하면 의외로 편함', '비견이 들어와서 옆자리랑 숟가락 속도 겹침. 마지막 만두 조심'],
  식신: ['식신이 들어온 날!! 오늘 뭘 먹어도 맛있는 날이야', '먹복 터진 날. 후식까지 가도 됨', '식신 만렙. 곱빼기 시켜도 사주가 허락함', '오늘 네 입은 미슐랭 심사위원 입이야. 뭘 먹어도 정확함'],
  재성: ['재물운이 들어옴. 근데 그 돈 점심값으로 나갈 운명', '돈 기운 최고조. 오늘은 네가 계산할 각', '재성이 강한 날. 법인카드가 네 쪽으로 기울고 있어', '지갑에 바람이 들어오는 날;; 더치페이 먼저 외쳐'],
  관성: ['관성의 날;; 오후에 일 터질 수 있으니 든든하게 먹어둬', '윗사람 기운이 강한 날. 자리는 구석 추천', '관성 뜸. 점심에 상사 마주칠 확률 높으니 동선 체크', '결재 기운이 감돌아. 밥이라도 든든해야 버틴다'],
  인성: ['인성의 날. 누가 사줄 수도 있으니 지갑 천천히 꺼내', '배울 게 들어오는 날. 새 메뉴 도전해봐', '인성이 들어와서 오늘은 얻어먹는 그림이 나옴. 사양 금지', '어른 기운이 널 감싸는 날. 국물 있는 걸로 대접받아'],
};
...
export function buildFortunes(sajus, today) {
  ...
  return sajus.map(mb => {
    ...
    return {
      ...
      remedy: `${mb.saju.lacking} 기운이 비었어. ${EL_FOOD[mb.saju.lacking]}`,
    };
  });
}
...
export function pickPayer(sajus, today) {
  ...
  return {
    name: w.name,
    reason: `오늘따라 ${w.name}한테 돈 들어올 기운이 제일 세. 그 돈, 점심값으로 나갈 운명이야!! 소문낼거야`,
